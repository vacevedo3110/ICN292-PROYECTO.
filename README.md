# Proyecto ICN292 

## 1. Contexto del Proyecto
* **Empresa:** "Inversiones Rumay Limitada"
* **Rubro:** Construcción RUT: 76.380.707-K
* Aclaración:** La constructora tiene de nombre "Constructora Altura". Sin embargo, opera ante el SII por el nombre de "Inversiones Rumay Ltda".
* **Problema:** Actualmente, el dueño realiza todos los cálculos de costos de forma manual. Esto genera una demora de aproximadamente de 3 días para entregar una cotización y un margen de error de entre 10% y 20% en la estimación de costos, lo que termina impactando en el tiempo de espera de clientes y del desarrollo de los distintos proyectos.
* **Objetivo del Sistema:** Automatizar el cálculo de los presupuestos y tener una base con los precios de materia prima (materiales, mano de obra, etc.) para reducir los tiempos de respuesta al cliente y evitar errores de presupuesto por fallas en el cálculo de precios y costos.

## 2. Organización del Repositorio
Entrega 1

* **`docs/`**: Documentación.
  * `00-caso-pyme.md`: Datos de la empresa.
  * `01-requerimientos.md`: Alcance, actores involucrados y requisitos.
  * `02-bpmn.md`: Explicación de los procesos actuales y propuestos.
  * `03-er-preliminar.md`: Justificación base de datos.
* **`assets/`**: Diagramas exportados y capturas de respaldo SII.
* **`informe/`**: Informe Entrega 1.
  * Informe PDF.
  * Archivo Word (.docx).

## 3. Proyección para la Entrega 2
* **Base de datos:** SQL.
* **Automatizacion:** n8n
* El modelo de datos planteados serán la base para desarrollar el prototipo en un entorno local host. 

## 4. Equipo de Trabajo
1. **VICENTE IGNACIO ACEVEDO CAMPOS** [Liderazgo del proyecto]  
3. **FERNANDO MIGUEL BEIZA ACUÑA**  [Modelado BPMN 2.0]
4. **THANNIA CONSTANZA CONTRERAS BASTIDAS** [Diseño Base de Datos]
6. **MATIAS IGNACIO GOMEZ RAMIREZ** [Software]
7. **BENJAMIN ANDRES LARA ORELLANA** [Acercamiento con la PYME]

## 5.  Accesos Rápidos
**[Ruta al Informe Final PDF](./informe/ICN292-Proyecto-E1.pdf)**
**[Enlace de la entrega en Aula](https://aula.usm.cl/mod/assign/view.php?id=8305258&action=view)**
